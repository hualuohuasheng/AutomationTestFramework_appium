...
class IRoomTest(unittest.TestCase):
    ...
    def test_001参与者多次加入离开房间(self):
        ...
        for driver in self.driverlist:
            ...
            proc = pool.apply_async(app_behavior.joinandleaveroom, (driver, self.roomid, self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_002参与者多次切换角色(self):
        ...
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(app_behavior.changerole, (driver, self.roomid,  self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_003参与者多次切后台(self):
        ...
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(app_behavior.backapp, (driver, self.roomid, self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
